...
Ref<RunLoop> RunLoop::FromThreadId(const Thread::Id &id) {
...
RunLoop::Status RunLoop::Run(const uint64_t repeat, const Duration timeout,
  ...
  RefPtr<Scope> previous_scope = guard->current_scope_;
...
void RunLoop::DoBlocks(RefPtr<Scope> scope) {
  auto scope_guard = scope->shared_.Lock();
  if (scope_guard->block_queue_.empty()) return;
  const auto func = scope_guard->block_queue_.front();
