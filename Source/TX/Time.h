...
class Duration final {
 public:
  ...
  constexpr Duration(const int64_t n) /* NOLINT(*-explicit-constructor) */
      : inner_(n) {}
  static Duration NanoSecond(const int64_t n) { return n; }
  static Duration MicroSecond(const int64_t n) { return n * 1000; }
  static Duration MilliSecond(const int64_t n) { return n * 1000000; }
  static Duration Second(const int64_t n) { return n * 1000000000; }
  static Duration Minute(const int64_t n) { return n * 1000000000 * 60; }
  static Duration Hour(const int64_t n) { return n * 1000000000 * 60 * 60; }
...
constexpr Duration operator""_ns(const unsigned long long n) {
  return Duration::NanoSecond(static_cast<int64_t>(n));
}
constexpr Duration operator""_us(const unsigned long long n) {
  return Duration::MicroSecond(static_cast<int64_t>(n));
}
constexpr Duration operator""_ms(const unsigned long long n) {
  return Duration::MilliSecond(static_cast<int64_t>(n));
}
constexpr Duration operator""_s(const unsigned long long n) {
  return Duration::Second(static_cast<int64_t>(n));
}
constexpr Duration operator""_m(const unsigned long long n) {
  return Duration::Minute(static_cast<int64_t>(n));
}
constexpr Duration operator""_h(const unsigned long long n) {
  return Duration::Hour(static_cast<int64_t>(n));
}
...
class Time final {
 public:
  explicit Time() : utc_(false) {
    ...
    if (static_cast<uint64_t>(wall_tp.sec) >> 33) {
      ...
      wall_ = static_cast<uint64_t>(wall_tp.nsec);
      ext_ = wall_tp.sec;
      return;
    }
    wall_ = kHasMono | (static_cast<uint64_t>(wall_tp.sec) << kNSecShift) |
            static_cast<uint64_t>(wall_tp.nsec);
    ext_ = mono;
  }

  explicit Time(const int64_t unix) : wall_(0), ext_(0), utc_(false) {
    const int64_t u_sec = unix / 1000000000;
    const int64_t u_nsec = unix % 1000000000;
  ...
  TX_NODISCARD DateTime DateTime() const {
    if (!zone_set) {
      tzset();
      zone_set = true;
    }
 ...
 private:
  ...
  TX_NODISCARD int64_t subMono(const int64_t t, const int64_t u) const {
    const int64_t d = t - u;
